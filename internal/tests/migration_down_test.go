...
func TestDowbMigrations(t *testing.T) {
...
`
	...
	deleteMigrateFromDBVersionReq := `DELETE from db_version 
	where version_id=$1;`
	_, err = m.Cfg.DB.Conn.Exec(context.Background(), deleteMigrateFromDBVersionReq, migrateNum)
	if err != nil {
		t.Fatalf("failed to delete test migration: %v", err)
	}
	deleteMigrateTabelsReq := `DROP TABLE IF EXISTS users12;`
	_, err = m.Cfg.DB.Conn.Exec(context.Background(), deleteMigrateTabelsReq)
	if err != nil {
		t.Fatalf("failed to delete test migration: %v", err)
	}
